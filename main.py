...
def test():
    ...
    # Regarde le premier enfant
        first_child_stop = list(stop['routes'].values())[0]

        if first_child_stop['route_type'] in [0, 3, 4]:
            icon = 'train' if first_child_stop['route_type'] == 0 else 'bus' if first_child_stop['route_type'] == 3 else 'ship'
            color = 'red' if icon == 'train' else 'lightgray' if icon == 'bus' else 'blue'
            ...
            # Ajout du marqueur avec le popup dans le cluster
            folium.Marker(location=[first_child_stop['stop_lat'], first_child_stop['stop_lon']],
                        popup=folium.Popup(iframe),
                        icon=folium.Icon(color=color, icon=icon, prefix='fa')
                        ).add_to(marker_cluster)
